...
module.exports = {
  ...
  // Returns an object representing the current state of checks for a pull
  // request
  async fetchPullRequestChecks(pullRequestId) {
    const pullRequestData = await this.call('pulls.get', {
      pull_number: pullRequestId,
    });
    const pullRequestSha = _.get(pullRequestData, 'head.sha', '');
    return await this.fetchCommitChecks(pullRequestSha);
  },
